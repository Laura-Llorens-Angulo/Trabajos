...
int main( int argc, char *argv[] ) {
  ...
  dimVectorLocal = dimVectorInicial/numProcs ; // tamaño vector local
  ...
  if(miId == 0){
    t1 = MPI_Wtime();
    prc = 0;
    for(int i = 0; i<dimVectorInicial; dimVectorLocal++){
      if(i == 0){
        for(int j = 0; j<dimVectorLocal; j++){
          vectorLocal[j] = vectorInicial [i+j];
        }
      }else{
        MPI_Send(&vectorInicial[i+j], 1, MPI_DOUBLE, prc, 88, MPI_COMM_WORLD);
      }
      prc++; 
    }
    ...
    for(int i = 0; i<dimVectorLocal; i++){
      MPI_Recv(&vectorInicial[i], 1, MPI_DOUBLE, 0, 88, MPI_COMM_WORLD);
    }
  }
